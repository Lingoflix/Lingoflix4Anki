...
class AnkiFeatureInjector(FeatureInjector):
    ...
    def __init__(self, feature_name: str, feature_script: Callable, target_platform:str):
        ...
        aqt.utils.showInfo(f"Injecting {self.feature_name} with {len(feature_script())} long script...")
    ...
    def _inject_in_template(self, cards: list[AnkiCard]):
        ...
        model_ids = set(card.note().model()["id"] for card in cards)
        models = [mw.col.models.get(mid) for mid in model_ids]
        for model in models:
    ...
    def apply_feature(self, cards):
        ...
        if self.target_platform == Platforms.DESKTOP:
            ...
            if self.target_platform == Platforms.ANDROID: 
                ...
                aqt.utils.showInfo(f"injecting For android")
                ...
                aqt.utils.showInfo(f"Unknown platform: {self.target_platform}")
